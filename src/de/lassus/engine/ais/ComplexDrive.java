...
public class ComplexDrive implements AI {
	...
	public double act() {
		...
		if (nearestCars[0] != null && nearestCars[1] != null) {
			double x1 = simMovement(nearestCars[0].getX(), nearestCars[0].getVelocity(),
					nearestCars[0].getAcceleration(), VORRAUSSICHT);
			double x2 = simMovement(nearestCars[1].getX(), nearestCars[1].getVelocity(),
					nearestCars[1].getAcceleration(), VORRAUSSICHT);
